      ...
      const addConfirmationBanner = ({
        ...
        if (name === 'confirmation' && params.json) {
          ...
          if (order) {
            ...
            const customerName = Object(_ecomplus_utils__WEBPACK_IMPORTED_MODULE_0__["fullName"])(customer);
            const date = new Date();
            let ccParam = `orderSellerID=${compreConfieStoreId}&orderPlatform=ecomplus` + `&orderTotalSpent=${order.amount.total}` + `&orderDeliveryTax=${order.amount.freight || 0}` + `&orderID=${order.number || order._id}` + `&consumerEmail=${customer.main_email}` + `&billingEmail=${customer.main_email}` + `&consumerName=${encodeURIComponent(customerName)}` + `&orderDate=${date.getDate()}/${date.getMonth() + 1}/${date.getFullYear()} | ` + `${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}`;
            ...
            if (customer.registry_type === 'p' && customer.doc_number) {
              ccParam += `&consumerCPF=${customer.doc_number}` + `&billingCPF=${payer && payer.doc_number || customer.doc_number}`;
            }
            ...
            if (customer.birth_date) {
              ...
              ccParam += `&consumerBirthDate=${day.toString().padStart(2, '0')}` + `/${month.toString().padStart(2, '0')}/${year}`;
            }
            ...
            if (items && items.length) {
              ...
              ccParam += `&productSKU=${skus}&productValue=${values}&productQuantity=${quantities}` + `&ProductName=${names}&ProductMktSaleID=${productMktSaleID}`;
            }
            ...
            if (order.shipping_lines && order.shipping_lines[0]) {
              ...
              if (shippingLine.delivery_time && shippingLine.delivery_time.days) {
                ccParam += `&orderDeliveryTime=${shippingLine.delivery_time.days}` + `&productDeliveryTime=${shippingLine.delivery_time.days}`;
              }

              ccParam += `&consumerZipcode=${shippingLine.to.zip}` + `&billingZipcode=${shippingLine.to.zip}`;
            }
            ...
            const appendBanner = () => {
              ...
              if ($confirmation) {
                $confirmation.insertAdjacentHTML('beforeend', `<a id="bannerEconfy"></a> <param id="TagEConfy" value="${ccParam}">`);
                const $script = document.createElement('script');
                $script.id = 'getData';
                $script.type = 'text/javascript';
                $script.src = 'https://banner.compreconfie.com.br/scripts/tagBanner.min.js' + `?${compreConfieStoreId}&lightbox=true`;
...
//# sourceMappingURL=chunk.c43924a25a3e279ee0af.js.map